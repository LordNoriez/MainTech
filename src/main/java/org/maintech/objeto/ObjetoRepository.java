...
public interface ObjetoRepository extends CrudRepository<Objeto, Integer> {
	...
	@Query(value="Select if(isnull(Egresos),if(isnull(Ingresos), '0',Ingresos),if(isnull(Ingresos), Egresos*-1, (Ingresos-Egresos))) as Stock, " + 
			...
			" group by objeto_id_objeto) as Ingres left join " +
			" (select sum(cantidad_movimiento) as Egresos, objeto_id_objeto " + 
			" from movimiento where tipo_movimiento_id_tipo_movimiento=2  " +
			" group by objeto_id_objeto) as Egres " +
			" on Egres.objeto_id_objeto=Ingres.objeto_id_objeto) on objeto.id_objeto = Ingres.objeto_id_objeto, area_empresa, categoria " + 
			" where objeto.is_active=1 and area_empresa.id_area_empresa=objeto.area_empresa_id_area_empresa and  " +
			" categoria.id_categoria=objeto.categoria_id_categoria;", nativeQuery=true)
