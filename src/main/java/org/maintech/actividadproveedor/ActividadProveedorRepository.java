...
public interface ActividadProveedorRepository extends CrudRepository<ActividadProveedor, Integer> {
	...
	@Query(value="select c.id_costo from actividad_proveedor as ap inner join costo as c on ap.id_costo = c.id_costo inner join proveedor as p on p.id_proveedor = ap.id_proveedor where c.fecha_fin_costo is null and ap.id_actividad = ?1 and ap.id_proveedor = ?2", nativeQuery=true)	
	Integer getLastCosto(Integer idActividad, Integer idProveedor);
